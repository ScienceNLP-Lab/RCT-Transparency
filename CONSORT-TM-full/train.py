	...
	if config.target == "[CLS]":
		if config.mode == "single":
			for batch in DataLoader(data, batch_size=config.eval_batch_size, shuffle=False, collate_fn=colloate_fn):
				...
				true_labels.extend(target.tolist()[:4])
				predicted_values.extend(combine.tolist()[:4])  # probability predictions 

				sid_single = batch.PMCID

				combine = np.where(combine.numpy() > 0.5, 1, 0)
				target_result.extend(target.tolist()[:4])
				valid_result.extend(combine.tolist()[:4])  # binary predictions
			...
			for batch in DataLoader(data, batch_size=config.eval_batch_size, shuffle=False, collate_fn=colloate_fn_contextual):
				model.eval()
				target = batch.labels
				combine = model(batch)
				loss = loss_fn(combine, target)
				...
				target_new = target.tolist()[:4]
				predicted_values_new = combine.tolist()[:4]
				for k in range(len(target.tolist()[:4])):
					if sum(target_new[k]) > 0.1:
						true_labels.append(target_new[k])
						predicted_values.append(predicted_values_new[k])
		...
		for batch in DataLoader(data, batch_size=config.eval_batch_size, shuffle=False, collate_fn=colloate_fn_contextual):
			...
			true_labels.extend(target.tolist()[:4])
			predicted_values.extend(combine.tolist()[:4])		
...
def train_bert(config):
	...
	for folder_no in range(5):
		...
		if config.mode == "contextual":
			...
			train_dataset, list_name = contextual_load(config.train_file, train_pmids, config, folder_no = folder_no, current_mode = "train")
			test_dataset, list_name = contextual_load(config.train_file, test_pmids, config, folder_no = folder_no, current_mode = "test")
		...
		if config.target == "[CLS]":
			if config.mode == "single": 
				...
				for epoch in range(config.max_epoch):
					...
					progress = tqdm.tqdm(total=batch_num, ncols=75, desc='Train {}'.format(epoch))
					optimizer.zero_grad()
					for batch_idx, batch in enumerate(DataLoader(train, batch_size=config.batch_size, shuffle=True, collate_fn=colloate_fn_contextual)):
						optimizer.zero_grad()
						model.train()
						prediction = model(batch)
						loss = Loss(prediction, batch.labels)
