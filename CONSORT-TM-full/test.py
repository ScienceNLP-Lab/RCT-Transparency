...
def evaluate(model, Loss, data, config, batch_num, epoch, name, list_name, base_folder_name, folder_no, log_dir=None):
...
def test(config):
    ...
    for subdirectories in all_subdirectories:
        ...
        if config.mode == "contextual":
            test_dataset, list_name = contextual_load(config.train_file, test_pmids, config,
                                                      folder_no=-1, current_mode="test")
            test = test_dataset
        else:
        ...
        sid, report, running_loss, target_result, valid_result, micro_auc = evaluate(model, Loss, test,config,test_batch_num,epoch,'TEST',list_name, base_folder_name=base_folder,folder_no=folder_no)
